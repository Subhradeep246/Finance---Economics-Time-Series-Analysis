...
axes[0, 0].bar(annual_data['Year'], annual_data['Close Price'], color='blue', alpha=0.7)
axes[0, 0].set_title('Average Stock Close Price (by Year)', fontweight='bold')
axes[0, 0].set_xlabel('Year')
axes[0, 0].set_ylabel('Price (USD)')
axes[0, 0].grid(axis='y', alpha=0.3)

axes[0, 1].bar(annual_data['Year'], annual_data['GDP Growth (%)'], color='red', alpha=0.7)
axes[0, 1].set_title('GDP Growth (%)', fontweight='bold')
axes[0, 1].set_xlabel('Year')
axes[0, 1].set_ylabel('Percentage')
axes[0, 1].grid(axis='y', alpha=0.3)

axes[1, 0].bar(annual_data['Year'], annual_data['Inflation Rate (%)'], color='green', alpha=0.7)
axes[1, 0].set_title('Inflation Rate (%)', fontweight='bold')
axes[1, 0].set_xlabel('Year')
axes[1, 0].set_ylabel('Percentage')
axes[1, 0].grid(axis='y', alpha=0.3)

axes[1, 1].bar(annual_data['Year'], annual_data['Unemployment Rate (%)'], color='orange', alpha=0.7)
